...
class MotionStrengthModule(nn.Module):
    ...
    def __init__(self, t_window=3, in_channels=3):
        ...
        # Initialize temporal filter for motion detection
    ...
    def _init_temporal_filter(self):
        ...
        # Better temporal difference filter for motion detection
        with torch.no_grad():
            # Initialize all weights to zero first
            self.temp_conv.weight.zero_()
            
            if self.temp_conv.weight.shape[2] == 3:  # t_window = 3
                # Create a stronger temporal difference filter
                # This computes: current_frame - previous_frame
...
class TemporalRTDETR(nn.Module):
    ...
    def __init__(self, 
        ...
        self.use_motion = use_motion and motion_module is not None
        ...
        if hasattr(self.backbone, 'model') and hasattr(self.backbone.model, 'base_layer'):
            ...
            if isinstance(base_layer, nn.Sequential) and len(base_layer) > 0:
                ...
                if isinstance(old_conv, nn.Conv2d):
                    ...
                    print("✅ Successfully modified DLA backbone for 4-channel input")
                    return
                    
        elif hasattr(self.backbone, 'base_layer'):
            # Direct DLA structure
            ...
            print("✅ Successfully modified ResNet backbone for 4-channel input")
        
        else:
            print("⚠️ Warning: Could not automatically modify backbone input channels")
            print("Please manually modify the first layer to accept 4 channels")
    ...
    def forward(self, x, targets=None):
        ...
        # Standard RT-DETR forward pass with 4-channel input
