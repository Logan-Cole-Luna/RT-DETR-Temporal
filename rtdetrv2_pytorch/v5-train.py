#!/usr/bin/env python3
"""
v5-train.py - Fixed validation issues from v4
Critical fix: Proper validation loop without errors
"""
...
class EnhancedMonitor:
    ...
    def __init__(self, output_dir, model_type_str):
        ...
        with open(self.log_file, 'w', encoding='utf-8') as f:
            f.write("UAV RT-DETR Training Log - v5 Fixed Validation\n")
            f.write("=" * 80 + "\n")
...
def calculate_detection_metrics(outputs, targets, confidence_threshold=0.3, iou_threshold=0.5):
        ...
        if len(pred_scores_confident) > 0:
            ...
            for pred_idx in sorted_indices:
                ...
                for gt_idx in range(len(gt_boxes)):
                    ...
                    iou = bbox_iou_single(pred_box, gt_box)
...
def main():
    print("Starting UAV RT-DETR Training v5 - Fixed Validation")
    ...
    output_dir = config.OUTPUT_DIR_FORMAT_STRING.format(model_type_str=model_type_str).replace("v4", "v5")
    
    resume_checkpoint_path = config.RESUME_CHECKPOINT_PATH
    
    monitor = EnhancedMonitor(output_dir, model_type_str)
    monitor.log("Starting UAV RT-DETR Training v5 - Fixed Validation")
    monitor.log("=" * 80)
    ...
    validation_epoch_interval = max(1, int(num_epochs * 0.05))  # Every 5%
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if (epoch + 1) % validation_epoch_interval == 0 or epoch == num_epochs - 1:
                try:
                    ...
                    monitor.log(f"Validation failed for epoch {epoch+1}: {val_error}")
            ...
            if val_metrics['f1'] > 0 or math.isfinite(val_metrics['loss']):
